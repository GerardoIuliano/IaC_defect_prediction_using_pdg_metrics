<?xml version='1.0' encoding='utf-8'?>
<graphml xmlns="http://graphml.graphdrawing.org/xmlns" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://graphml.graphdrawing.org/xmlns http://graphml.graphdrawing.org/xmlns/1.0/graphml.xsd"><key id="d19" for="edge" attr.name="back" attr.type="string"/>
<key id="d18" for="edge" attr.name="transitive" attr.type="string"/>
<key id="d17" for="edge" attr.name="keyword" attr.type="string"/>
<key id="d16" for="edge" attr.name="id" attr.type="string"/>
<key id="d15" for="edge" attr.name="type" attr.type="string"/>
<key id="d14" for="node" attr.name="action" attr.type="string"/>
<key id="d13" for="node" attr.name="identifier" attr.type="string"/>
<key id="d12" for="node" attr.name="non_idempotent_components" attr.type="string"/>
<key id="d11" for="node" attr.name="expr" attr.type="string"/>
<key id="d10" for="node" attr.name="value" attr.type="string"/>
<key id="d9" for="node" attr.name="type" attr.type="string"/>
<key id="d8" for="node" attr.name="scope_level" attr.type="string"/>
<key id="d7" for="node" attr.name="value_version" attr.type="string"/>
<key id="d6" for="node" attr.name="version" attr.type="string"/>
<key id="d5" for="node" attr.name="name" attr.type="string"/>
<key id="d4" for="node" attr.name="location" attr.type="string"/>
<key id="d3" for="node" attr.name="node_id" attr.type="string"/>
<key id="d2" for="node" attr.name="node_type" attr.type="string"/>
<key id="d1" for="graph" attr.name="role_version" attr.type="string"/>
<key id="d0" for="graph" attr.name="role_name" attr.type="string"/>
<graph edgedefault="directed"><data key="d0">{"path": "ansible-role-degoss/tasks/main.yml", "id" : "98"}</data>
<data key="d1">latest</data>
<node id="6">
  <data key="d2">Variable</data>
  <data key="d3">6</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 4, "column": 1, "includer_location": null}</data>
  <data key="d5">"degoss_tmp_root"</data>
  <data key="d6">0</data>
  <data key="d7">0</data>
  <data key="d8">14</data>
</node>
<node id="7">
  <data key="d2">Literal</data>
  <data key="d3">7</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 4, "column": 18, "includer_location": null}</data>
  <data key="d9">"str"</data>
  <data key="d10">"/tmp/degoss"</data>
</node>
<node id="9">
  <data key="d2">Variable</data>
  <data key="d3">9</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 6, "column": 1, "includer_location": null}</data>
  <data key="d5">"degoss_goss_install_dir"</data>
  <data key="d6">0</data>
  <data key="d7">0</data>
  <data key="d8">14</data>
</node>
<node id="10">
  <data key="d2">Variable</data>
  <data key="d3">10</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 7, "column": 1, "includer_location": null}</data>
  <data key="d5">"degoss_goss_bin"</data>
  <data key="d6">0</data>
  <data key="d7">0</data>
  <data key="d8">14</data>
</node>
<node id="17">
  <data key="d2">Variable</data>
  <data key="d3">17</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 13, "column": 1, "includer_location": null}</data>
  <data key="d5">"goss_output_format"</data>
  <data key="d6">0</data>
  <data key="d7">0</data>
  <data key="d8">14</data>
</node>
<node id="18">
  <data key="d2">Literal</data>
  <data key="d3">18</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 13, "column": 21, "includer_location": null}</data>
  <data key="d9">"str"</data>
  <data key="d10">"rspecish"</data>
</node>
<node id="25">
  <data key="d2">Variable</data>
  <data key="d3">25</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 20, "column": 1, "includer_location": null}</data>
  <data key="d5">"goss_file"</data>
  <data key="d6">0</data>
  <data key="d7">0</data>
  <data key="d8">14</data>
</node>
<node id="26">
  <data key="d2">Literal</data>
  <data key="d3">26</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 20, "column": 12, "includer_location": null}</data>
  <data key="d9">"str"</data>
  <data key="d10">"goss.yml"</data>
</node>
<node id="75">
  <data key="d2">Expression</data>
  <data key="d3">75</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 6, "column": 26, "includer_location": null}</data>
  <data key="d11">"{{ degoss_tmp_root }}/bin"</data>
  <data key="d12">[]</data>
</node>
<node id="76">
  <data key="d2">IntermediateValue</data>
  <data key="d3">76</data>
  <data key="d13">11</data>
</node>
<node id="77">
  <data key="d2">Expression</data>
  <data key="d3">77</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/vars/main.yml", "line": 7, "column": 18, "includer_location": null}</data>
  <data key="d11">"{{ degoss_goss_install_dir }}/goss"</data>
  <data key="d12">[]</data>
</node>
<node id="78">
  <data key="d2">IntermediateValue</data>
  <data key="d3">78</data>
  <data key="d13">12</data>
</node>
<node id="84">
  <data key="d2">Expression</data>
  <data key="d3">84</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/tasks/main.yml", "line": 35, "column": 11, "includer_location": null}</data>
  <data key="d11">"{{ degoss_goss_bin }}"</data>
  <data key="d12">[]</data>
</node>
<node id="85">
  <data key="d2">IntermediateValue</data>
  <data key="d3">85</data>
  <data key="d13">14</data>
</node>
<node id="89">
  <data key="d2">Loop</data>
  <data key="d3">89</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/tasks/main.yml", "line": 42, "column": 15, "includer_location": null}</data>
</node>
<node id="98">
  <data key="d2">Task</data>
  <data key="d3">98</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/tasks/main.yml", "line": 46, "column": 3, "includer_location": null}</data>
  <data key="d14">"goss"</data>
  <data key="d5">"run tests"</data>
</node>
<node id="99">
  <data key="d2">Expression</data>
  <data key="d3">99</data>
  <data key="d4">{"file": "unknown file", "line": -1, "column": -1, "includer_location": null}</data>
  <data key="d11">"{{ goss_file }}"</data>
  <data key="d12">[]</data>
</node>
<node id="100">
  <data key="d2">IntermediateValue</data>
  <data key="d3">100</data>
  <data key="d13">19</data>
</node>
<node id="101">
  <data key="d2">Expression</data>
  <data key="d3">101</data>
  <data key="d4">{"file": "unknown file", "line": -1, "column": -1, "includer_location": null}</data>
  <data key="d11">"{{ goss_output_format }}"</data>
  <data key="d12">[]</data>
</node>
<node id="102">
  <data key="d2">IntermediateValue</data>
  <data key="d3">102</data>
  <data key="d13">20</data>
</node>
<node id="103">
  <data key="d2">Variable</data>
  <data key="d3">103</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/tasks/main.yml", "line": 50, "column": 13, "includer_location": null}</data>
  <data key="d5">"goss_output"</data>
  <data key="d6">0</data>
  <data key="d7">0</data>
  <data key="d8">18</data>
</node>
<node id="108">
  <data key="d2">Conditional</data>
  <data key="d3">108</data>
  <data key="d4">{"file": "/mnt/c/Users/gerar/Desktop/IaC_defect_prediction_using_pdg_metrics/input/repositories/ansible-role-degoss/tasks/main.yml", "line": 55, "column": 9, "includer_location": null}</data>
</node>
<edge source="6" target="75">
  <data key="d15">USE</data>
  <data key="d16">6-75-0</data>
</edge>
<edge source="7" target="6">
  <data key="d15">DEF</data>
  <data key="d16">7-6-0</data>
</edge>
<edge source="9" target="77">
  <data key="d15">USE</data>
  <data key="d16">9-77-0</data>
</edge>
<edge source="10" target="84">
  <data key="d15">USE</data>
  <data key="d16">10-84-0</data>
</edge>
<edge source="17" target="101">
  <data key="d15">USE</data>
  <data key="d16">17-101-0</data>
</edge>
<edge source="18" target="17">
  <data key="d15">DEF</data>
  <data key="d16">18-17-0</data>
</edge>
<edge source="25" target="99">
  <data key="d15">USE</data>
  <data key="d16">25-99-0</data>
</edge>
<edge source="26" target="25">
  <data key="d15">DEF</data>
  <data key="d16">26-25-0</data>
</edge>
<edge source="75" target="76">
  <data key="d15">DEF</data>
  <data key="d16">75-76-0</data>
</edge>
<edge source="76" target="9">
  <data key="d15">DEF</data>
  <data key="d16">76-9-0</data>
</edge>
<edge source="77" target="78">
  <data key="d15">DEF</data>
  <data key="d16">77-78-0</data>
</edge>
<edge source="78" target="10">
  <data key="d15">DEF</data>
  <data key="d16">78-10-0</data>
</edge>
<edge source="84" target="85">
  <data key="d15">DEF</data>
  <data key="d16">84-85-0</data>
</edge>
<edge source="85" target="98">
  <data key="d15">KEYWORD</data>
  <data key="d17">"args.executable"</data>
  <data key="d16">85-98-0</data>
</edge>
<edge source="89" target="98">
  <data key="d15">ORDER</data>
  <data key="d18">false</data>
  <data key="d19">false</data>
  <data key="d16">89-98-0</data>
</edge>
<edge source="98" target="103">
  <data key="d15">DEF</data>
  <data key="d16">98-103-0</data>
</edge>
<edge source="98" target="108">
  <data key="d15">ORDER</data>
  <data key="d18">false</data>
  <data key="d19">false</data>
  <data key="d16">98-108-0</data>
</edge>
<edge source="99" target="100">
  <data key="d15">DEF</data>
  <data key="d16">99-100-0</data>
</edge>
<edge source="100" target="98">
  <data key="d15">KEYWORD</data>
  <data key="d17">"args.path"</data>
  <data key="d16">100-98-0</data>
</edge>
<edge source="101" target="102">
  <data key="d15">DEF</data>
  <data key="d16">101-102-0</data>
</edge>
<edge source="102" target="98">
  <data key="d15">KEYWORD</data>
  <data key="d17">"args.format"</data>
  <data key="d16">102-98-0</data>
</edge>
</graph></graphml>